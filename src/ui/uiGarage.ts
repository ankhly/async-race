...
export function renderCar(name:string, color:string, id:number):string {
  const carsPage = document.querySelector('.car-page') as HTMLElement;
  const blockCar = `
<div data-id=${id} class="car-page__body">
<div class="car-page__settings settings-car">
  <button class="settings-car__select btn">Select</button>
  <button class="settings-car__remove btn">Remove</button>
  <div class="settings-car__name">${name}</div>
</div>
<div class="car-page__play play-car">
  ...
  carsPage.insertAdjacentHTML('beforeend', blockCar);
  return blockCar;
}
